...
**v97** — 198 bugs fixed · 283+ features · ~63 950 LoC · 263 files
...
Current: **v97** — 198 bugs fixed · 283+ features · ~63 950 LoC · 263 files

*v97 highlights:* the **Eclipsa/IAMF authoring** flow had been writing non-conformant files since v88. Each file was missing its mix description — the part that tells a player how to combine the channels. Nothing looked wrong: the file was created without error and the suite read its channel layout back correctly. The cause was a single character in the command sent to ffmpeg — a comma where a colon belonged — which made ffmpeg discard the entire mix block. ffmpeg did warn about it, but the message was hidden by the setting that only lets serious errors through. Players and ffmpeg up to 9.0 accepted such files, so the defect stayed invisible; **ffmpeg 9.0 rejects them outright**, as the IAMF specification requires at least one submix. Fixed across all four channel layouts (stereo, 5.1, 7.1, 7.1.4) on both platforms; the stereo case needed a second correction, a missing mix label on the element — which turns out to be the very reason the wrong form was chosen back in v88. The flow now also **verifies the file it just produced**: if the mix description is missing, the file is not delivered and the reason is stated, instead of trusting an exit code that in this case said nothing. Files authored with v88–v96 still play on current players but should be re-created before use with newer software. The same release also opens **media analysis to audio files**: option 3 used to skip anything without a picture, which meant the audio-only output of option 2 — Eclipsa included — could not be checked with the suite's own analyzer. Audio files are now reported in full (codec with its Atmos / DTS:X / Eclipsa label, bitrate, sample rate, depth, channels and layout, language, track count, subtitles, chapters, attachments), standalone audio formats are recognized (m4a, mka, flac, opus, wav, aac, ac3, eac3, dts, thd, iamf), and picture fields read `N/A` instead of inventing plausible-but-false values — previously an audio file would have shown an empty resolution, the whole-file bitrate reported as video bitrate, and "SDR" on something with no picture. The CSV keeps its 38 columns, so anything consuming the report still works. Suites: **Linux 102/102 with no skips**, Windows bash 100/0/2 of 102, Windows PowerShell 94/94.
