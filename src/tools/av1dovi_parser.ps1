<#
.SYNOPSIS
    Installer & Updater pentru av1dovi_tool pe Windows (sven-pke fork).
.DESCRIPTION
    Cloneaza si compileaza sven-pke/dovi_tool (fork al quietvoid/dovi_tool cu
    suport AV1). Folosit de av_encode.ps1 pentru injectare RPU Dolby Vision in
    fluxuri AV1.

    Binarul se instaleaza ca av1dovi_tool.exe (rename pentru a evita coliziunea
    cu dovi_tool.exe upstream, care ramane HEVC-only).
...
$RepoUrl     = "https://github.com/sven-pke/dovi_tool.git"
$InstallRoot = Join-Path $env:USERPROFILE "av1dovi_tool"
$SrcBinName  = "dovi_tool.exe"
$DestBinName = "av1dovi_tool.exe"
$TargetPath  = Join-Path $PSScriptRoot $DestBinName

Write-Host ""
Write-Host "╔══════════════════════════════════════════════╗" -ForegroundColor Cyan
Write-Host "║ AV1 DOVI_TOOL INSTALLER (sven-pke fork)      ║" -ForegroundColor Cyan
Write-Host "╚══════════════════════════════════════════════╝" -ForegroundColor Cyan
